    ...
    <section className="">
      <Slider {...settings}>
        {images.map((slide, index) => (
          <div
            key={index}
            className="md:h-[90vh] h-[400px] flex-shrink-0 relative slide-container"
          >
            <img
              src={slide}
              alt={`Slide ${index}`}
              className="w-full  object-cover slide-image"
            />
            <div className="md:px-12 px-6 absolute h-full inset-0 bg-black bg-opacity-30 flex flex-col text-white justify-center item-center">
              <h1 className="md:text-4xl text-3xl lg:text-6xl font-bold mb-4 text-center">
                Project Update
              </h1>
            </div>
          </div>
