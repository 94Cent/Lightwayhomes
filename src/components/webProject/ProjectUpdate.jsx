const ProjectUpdate=()=>{
    return(
        <section className="mt-20 text-center bg-gray-300 text-lg h-80 flex items-center justify-center flex-col gap-y-10 font-medium text-purple px-3">
        <h2 className="text-2xl">Projects Updates</h2>
        <p className="font-normal">You can check our updates on Roman height project</p>
        <button className="border border-purple py-3 sm:w-[27rem] w-full">Get it Now</button>
    </section>
    )
}
export default ProjectUpdate